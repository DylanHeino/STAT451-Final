...
merged_data <- merge(filtered_education_data, crime_data, by = "state")
...
server <- function(input, output) {
  ...
  correlation_text <- reactive({
    correlation <- cor.test(merged_data$MostEducatedStatesTotalBSDegreeOrHigher, 
  ...
  output$crime_plot <- renderPlotly({
    ...
    cor_value <- round(cor(merged_data$MostEducatedStatesTotalBSDegreeOrHigher, crime_data[[input$crime_type]]), 3)
    

    p <- ggplot(merged_data, aes_string(x = "MostEducatedStatesTotalBSDegreeOrHigher", y = input$crime_type)) +
